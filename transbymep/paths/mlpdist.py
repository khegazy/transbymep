...
class MLPDistpath(BasePath):
    ...
    def geometric_path(self, time: float, *args):
        ...
        """
        return self.dist_to_geo(
            self.mlp(time) \
            - (1 - time) * self.mlp(torch.tensor([[0.]], device=self.device)) \
            - (time) * self.mlp(torch.tensor([[1.]], device=self.device)) \
            + self.interpolation(time, mode='internal'))
    
    def interpolation(self, time: torch.tensor, mode: str = 'cartesian') -> torch.tensor:
        """
        Interpolates between the initial and final points.

        Args:
            time (torch.Tensor): Time parameter for generating the path.
            mode (str, optional): Interpolation mode. Defaults to 'cartesian'.

        Returns:
            torch.Tensor: The interpolated path.
        """
        if mode == 'cartesian':
            return self.geo_to_dist((1 - time) * self.initial_point + time * self.final_point)
        elif mode == 'internal':
            return (1 - time) * self.geo_to_dist(self.initial_point) + time * self.geo_to_dist(self.final_point)
        else:
